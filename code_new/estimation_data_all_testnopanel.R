...
WeeklyEstimationData <- function(x) {
  ...
  x %>% 
    group_by(ISOWEEK = ISOweek(PURCHASE_DATE)) %>% 
    summarise(AVG_SPENT_DS_DEF = 
                median(TOTAL_SPENT_DS_DEF)) %>%
    ungroup() %>% 
    arrange(ISOWEEK) %>% 
    mutate(Y = 
             log(AVG_SPENT_DS_DEF) 
           - log(lag(AVG_SPENT_DS_DEF,
                     n = lag_in_weeks)),
           Z1 = 
             lag(Y, n = 2)) %>%
    #na.exclude() %>% 
    ...
    transmute(DATE = as.Date(ISOweek2date(paste(ISOWEEK, "1", sep = "-"))),
              HOUSEHOLD = case_when(region == "NE" ~ 1,
                                    region == "MW" ~ 2,
                                    region == "SO" ~ 3,
                                    region == "WE" ~ 4),
              Y = Y,
...
MonthlyEstimationData <- function(x) {
  ...
  x %>% 
    group_by(YEAR = year(PURCHASE_DATE),
...
if (FALSE) {
  ...
  test <-
    ...
    summarise(AVERAGE = mean(TOTAL_SPENT_DS_DEF),
              MEDIAN = median(TOTAL_SPENT_DS_DEF)) %>%
    ungroup() %>% 
    mutate(DIFF = AVERAGE - MEDIAN)
...
count <- 
  log_est %>% 
  group_by(HOUSEHOLD) %>% 
  summarise(NUM_QUARTER = n_distinct(DATE))

hist(count$NUM_QUARTER, breaks=seq(0,190,l=190), xlim = c(0,190))
}