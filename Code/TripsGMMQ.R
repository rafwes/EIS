...
Trips4 <- Trips4[c('household_code', 'week', 'month', 'weekR', 'monthR', 'year', 'Y', 'LogR', 'YInst', 'Lag2LogNomR', 'Lag2Inf', 'Age', 'Edu', 'household_income', 'FSChange')]
...
#             error=function(w)list(b=c(NA,NA),h=NA))
    ...
    # g1 = derivative wrt beta : 1/beta * 1/gamma
    # g2 is application of delta method
    # varaince for gamma comes directly from what it is
    # This thing is just used to find the varaince of beta
    # g2 = derivative wrt gamma : ln(beta)*1/gamma^2
    # g3 = derivative wrt : 0
    ...
    ###########################    
        ...
        print(head(L))
        print(head(array(data=Itilde.deriv(-L/h),dim=dim(Z)) * Z))
        #long
        #print(head(Z))
        print(head(Ld))
