...
Trips4 <- read.csv('Datasets/RealTripsChangesMonthlyIR.csv')
...
Trips4_1 <- Trips4[which(Trips4$monthR >= Beg3 & Trips4$monthR <= End3),]
...
PLM <- plm(Y ~ LogR | YInst + Lag2LogNomR + Lag2Inf, data=Trips4_1, model='pooling', index=c('household_code', 'monthR'))
summary(PLM)
...
write.csv(QGMMResults, "EIS/Output/QGMM_New_PooledResultsP3_Monthly.csv", row.names=FALSE)
