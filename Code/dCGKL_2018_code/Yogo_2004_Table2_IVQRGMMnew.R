...
library("GenSA")
SEE.FLAG <- TRUE #check Kaplan and Sun (2017) "optimal" bandwidth?
SEE.FLAG <- SEE.FLAG && 
  tryCatch({source(paste0(CODE.DIR,"/ivqr_see.R"));TRUE},
...
for (icountry in 1:length(COUNTRIES)) {
  ...
  hs.used[[COUNTRY]] <- data.frame(GMM=rep(NA,length(TAUS)), MM=NA, SEE=NA)
  # 
  for (itau in 1:length(TAUS)) {
    ...
    if (LOGLINEAR.FLAG) {
      ...
      h.see <- NA
      if (SEE.FLAG) {
        ret <- ivqr_see(p=tau, Y=Y, X=cbind(D,X.excl), Z=cbind(X.excl,Z.excl))
        hs.used[[COUNTRY]][which(TAUS==tau),'SEE'] <- h.see <- ret$hhat #actually this is the "optimal," not "used"
      }
      # 
      # GMM
      ret <- ivqr.gmm(tau=tau,  Y=Y, X.excl=X.excl, Z.excl=Z.excl, D=D, h=ifelse(SEE.FLAG && !is.na(h.see), h.see, H), dB=2,
      ...
      ret.gmmq <- tryCatch(gmmq(tau=tau, dB=2, Y=cbind(Y,D), X=X.excl, Z.excl=Z.excl,
                                ...
                                h=ifelse(SEE.FLAG && !is.na(h.see), h.see, H), VERBOSE=FALSE, RETURN.Z=FALSE, b.init=rbind(1/6,0)),
