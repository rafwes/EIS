...
# [Note: to run parallelized, must select-all/run instead of source()]
# Question? Comments? Need help? kaplandm@missouri.edu
# This and other code at: https://faculty.missouri.edu/~kaplandm

rm(list=ls()) #clear workspace
SAVE.FLAG <- TRUE ##########
TEST.FLAG <- FALSE #hypothesis testing
PARALLEL <- 4 # Fastest (for me): PARALLEL>1, power=Balanced
if (SAVE.FLAG) { NREP <- 1000; BREP <- 299 } else { NREP <- 3; BREP <- 1 } #BREP=299; 8min/rep (DGP1) if BREP=399, n=1e4. n=1e4,BREP=100,NREP=200 should be ~3days, but (bootstrap) simulation error is much higher, not very informative.
TAUs <- 1:2/4
...
if (PARALLEL>1) {
  if (!require("parallel") || !require("foreach") || !require("doRNG")) {warning("Install packages foreach, paralle, and doRNG in order to run PARALLEL."); PARALLEL <- 1}
...
OUTFILE.robRMSE2 <- paste0(sprintf("SIVQR_sims_robustRMSE2_NREP%d_",NREP),
                           format(Sys.time(),"%Y_%m_%d"),".txt")
if (TEST.FLAG) OUTFILE.inf <- paste0(sprintf("SIVQR_sims_inf_NREP%d_BREP%d_",NREP,BREP),
                      format(Sys.time(),"%Y_%m_%d"),".txt")
...
\\caption{\\label{tab:sim-RMSE1}Simulation results for estimators' RMSE, $\\num{%d}$ replications. ``S(G)MM'' is the estimator in \\cref{eqn:def-est-MM,eqn:def-est-GMM} (equivalent here due to exact identification); ``QR'' is quantile regression (no IV); ``IV'' is the usual (conditional mean) IV estimator.}
...
cat(sprintf("\\begin{table}[htbp]\n\\centering
\\caption{\\label{tab:sim-robust-RMSE2}Simulation results for smoothed (G)MM estimators' ``robust RMSE'' (replacing bias with median bias, replacing standard deviation with IQR/1.349) with different bandwidths; $\\num{%d}$ replications. $h_0$ is the smallest numerically feasible bandwidth.}
...
for (iDGP in 1:length(DGPs)) {
  ...
  for (irep in 1:NREP) {
    ...
    for (itau in 1:length(DGP$taus)) {
      if (PARALLEL>1) {
        ...
        bp.raw <- foreach(i=1:length(DGP$ns),.combine=cbind,.inorder=TRUE) %dorng% { #for non-parallel: comment
          ...
          if (is.na(gmmq.ret[1])) {
            ...
            slice <- c(rep(NA,4), c(beta.hats.par))
            if (TEST.FLAG) slice[1:4] <- c(W.hat, pval.W, pval.BSt, pval.BS)
  ...
  # } else if (DGP$outfmt==1) {
    ...
    RMSE.legtext <- c(sprintf("S(G)MM(%4.2f)",DGP$taus[1]),
                 sprintf("S(G)MM(%4.2f)",DGP$taus[2]),
                 sprintf("QR(%4.2f)",DGP$taus[1]),
                 sprintf("QR(%4.2f)",DGP$taus[2]),
                 sprintf("IV(%4.2f)",DGP$taus[1]),
                 sprintf("IV(%4.2f)",DGP$taus[2]))
    if (length(DGP$taus)==1) legtext <- c(sprintf("S(G)MM(%4.2f)",DGP$taus[1]), sprintf("QR(%4.2f)",DGP$taus[1]), sprintf("IV(%4.2f)",DGP$taus[1]))
...
if (TEST.FLAG) {t2s <- W.hats[,1,nind]; hist(ifelse(abs(t2s)<15,t2s,-1), freq=FALSE, breaks=15); curve(dchisq(x,df=1), col=2,lty=2,lwd=2,add=TRUE)}

cat("\\bottomrule\n\\end{tabular}\n\\end{table}",
    file=OUTFILE.RMSE1,sep='\n',append=TRUE)
cat(sprintf("\\bottomrule\n\\end{tabular}\n%%\n\\begin{tablenotes}
  \\item $\\num{%d}$ replications. ``S(G)MM'' is the estimator in \\cref{eqn:def-est-MM,eqn:def-est-GMM} (equivalent here due to exact identification); ``QR'' is quantile regression (no IV); ``IV'' is the usual (mean) IV estimator. 
\\end{tablenotes}\n\\end{threeparttable}
\\end{table}", NREP),
